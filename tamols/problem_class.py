...
from .numpy_compat import ravel_pytree, vmap, jit, random
from .helpers import (
...
class TAMOLS():
    def __init__(self, gait: Gait, terrain: Terrain, robot: Robot,
        ...
        self.current_state = CurrentState(
            ...
            use_virtual_floor=np.array(False),
        )

        # --- Assertions ---
        assert isinstance(self.gait.tau_k, (list, tuple, np.ndarray, np.ndarray)), \
        ...
        self._two_contact_phases = np.asarray(two_contact_phase_idx, dtype=np.int32)
        self._two_contact_pairs  = np.asarray(two_contact_pairs, dtype=np.int32) if two_contact_pairs else np.zeros((0,2), dtype=np.int32)
        ...
        self._multi_contact_phase_pairs = (
            np.asarray(multi_pairs, dtype=np.int32)
            if multi_pairs else np.zeros((0, 3), dtype=np.int32)
        )

        # Precompute heightmap gradients
        self.h_s1 = np.array(get_hs1(terrain.heightmap))
        self.h_s2 = np.array(get_hs2(terrain.heightmap))
        ...
        self.grad_h_x = np.array(gxh)
        self.grad_h_y = np.array(gyh)
        self.grad_h_s1_x = np.array(gx1)
        self.grad_h_s1_y = np.array(gy1)
        self.laplacian_h = np.array(get_laplacian(terrain.heightmap, self.terrain.grid_cell_length))
        ...
        self.grad_h2_x = np.array(gx2)
        self.grad_h2_y = np.array(gy2)
        ...
        tpl = {
            "a_pos": np.zeros((self.gait.n_phases, 3, self.gait.spline_order + 1)),
            "a_rot": np.zeros((self.gait.n_phases, 3, self.gait.spline_order + 1)),
            "p_1": robot.p_1_start,
            "p_2": robot.p_2_start,
            "p_3": robot.p_3_start,
            "p_4": robot.p_4_start,
            "slack_var": np.ones(self.gait.n_phases)
        }
    ...
    def _cost_of_phase(self, a_pos_phase, a_rot_phase, timesteps_phase, limb_center, T_k_phase):
        return np.sum(vmap(lambda t: self._cost_at_t(a_pos_phase, a_rot_phase, t, limb_center, T_k_phase))(timesteps_phase))

    # ----------------- x-only objective -----------------
    def compute_objective(self, x: np.ndarray, cs: CurrentState) -> np.ndarray:
        sv = self._unravel(x)

        # Coerce measured feet to NumPy arrays once (fixed shapes/dtypes)
        m1 = np.asarray(cs.p_1_meas)
        m2 = np.asarray(cs.p_2_meas)
        m3 = np.asarray(cs.p_3_meas)
        m4 = np.asarray(cs.p_4_meas)

        # Decision variables
        a_pos = sv["a_pos"]  # (n_phases, 3, k+1)
        a_rot = sv["a_rot"]  # (n_phases, 3, k+1)
        feet = np.stack([sv["p_1"], sv["p_2"], sv["p_3"], sv["p_4"]])  # (4,3)
        feet_prev = np.stack([m1, m2, m3, m4])  # (4,3)
        w = self.gait.weights

        # Select foothold heightmap by cs.use_virtual_floor (no re-JIT needed)
        hmap_real = np.asarray(self.terrain.heightmap)
        hmap = np.where(cs.use_virtual_floor, self.h_s2, hmap_real)

        # Select gradient fields based on use_virtual_floor
        grad_h_x   = np.where(cs.use_virtual_floor, self.grad_h2_x,        self.grad_h_x)
        grad_h_y   = np.where(cs.use_virtual_floor, self.grad_h2_y,        self.grad_h_y)
        grad_h1_x  = np.where(cs.use_virtual_floor, self.grad_h2_x,   self.grad_h_s1_x)
        grad_h1_y  = np.where(cs.use_virtual_floor, self.grad_h2_y,   self.grad_h_s1_y)
        ...
        phase_idx = np.arange(self.gait.n_phases)

        def limb_phase_sum(a_pos_all, a_rot_all, limb_center):
            return np.sum(vmap(lambda n: self._cost_of_phase(a_pos_all[n], a_rot_all[n],
        ...
        at_des = np.asarray(self.gait.at_des_position, dtype=feet.dtype)  # (n_phases,4)
        ...
        contact = np.asarray(self.gait.contact_schedule, dtype=feet.dtype)  # (n_phases,4)
        ...
        def kin_mid_phase(a_pos_n, a_rot_n, timesteps_n, feet_sel_n, contact_n):
            ...
            return w[2] * np.sum(per_leg)

        kin_mid_total = np.sum(vmap(kin_mid_phase)(
        ...
        pair_cost_total = np.sum(pair_costs_all)

        return (np.sum(static_costs)
                + np.sum(spline_costs)
                + kin_mid_total
                + pair_cost_total
    ...
    def compute_eq_constraints(self, x: np.ndarray, cs: CurrentState) -> np.ndarray:
        sv = self._unravel(x)
        ...
        if self.gait.n_phases > 1:
            jcs = vmap(junction)(np.arange(self.gait.n_phases - 1)).reshape(-1)
            eq = np.concatenate([p0_err, r0_err, v0_err, w0_err, jcs])
        else:
            eq = np.concatenate([p0_err, r0_err, v0_err, w0_err])

        return eq
    
    # ----------------- x-only inequality constraints -----------------
    def compute_ineq_constraints(self, x: np.ndarray, cs: CurrentState) -> np.ndarray:
        sv = self._unravel(x)
        ...
        def az_minus_g_phase(phase):
            ...
            def at_t(t):
                ...
                return acc[2] - g[2]
            return vmap(at_t)(ts)
        az_g = np.concatenate([az_minus_g_phase(p) for p in range(self.gait.n_phases)])
        ...
        def friction_phase(phase):
            ...
            return vmap(at_t)(ts)
        fric = np.concatenate([friction_phase(p) for p in range(self.gait.n_phases)])
        ...
        def dyn_phase_eq(phase, p_i, p_j):
            ...
            ts = np.array([0.0, Tk/2, Tk], dtype=Tk.dtype)
            p_ij = p_j - p_i
        ...
        dyn_eq = np.concatenate(dyn_eq_list) if dyn_eq_list else np.zeros((0,), dtype=feet.dtype)

        def dyn_phase_ineq(phase, p_i, p_j):
            a_pos, a_rot = sv["a_pos"][phase], sv["a_rot"][phase]
            ts = np.array([self._timesteps[phase][-1]])
            p_ij = p_j - p_i
            eps = sv["slack_var"][phase]
            ...
            def at_t(t):
                ...
                return np.array(np.dot(p_ij, L_B_dot) - mass * scalar_triple_product(p_ij, p_B - p_i, a_B) + eps)
        ...
        dyn_ineq = (np.concatenate(dyn_ineq_list)
                    if dyn_ineq_list else np.zeros((0,), dtype=feet.dtype))
        ...
        def giac_block(p1, p2, p3, p4):
            ...
            return  np.array([
                np.dot(np.cross(p_13, p_12), e_z),
                np.dot(np.cross(p_14, p_13), e_z),
                np.dot(np.cross(p_24, p_23), e_z),
                np.dot(np.cross(p_21, p_24), e_z),
            ])
        ...
        def kin_phase(phase):
            ...
            def at_t(t):
                ...
                def per_leg(foot, limb_center):
                    ...
                    return np.array([ll2 - l_min**2, l_max**2 - ll2])  # >= 0
        ...
        kin = np.concatenate([kin_phase(p) for p in range(self.gait.n_phases)])

        # Slack vars >= 0
        slack_var_constr = sv["slack_var"]  # (n_phases,)

        return np.concatenate([fric, kin, az_g, slack_var_constr, dyn_ineq, dyn_eq, giac])
    
    # ============== MPPI-specific methods ==============
    def compute_total_cost(self, x: np.ndarray, cs: CurrentState) -> np.ndarray:
        ...
        eq_penalty = self.w_eq * np.sum(c_eq ** 2)
        ...
        ineq_violation = np.maximum(0.0, -c_ineq)  # ReLU(-g(x))
        ineq_penalty = self.w_ineq * np.sum(ineq_violation ** 2)
        
        return obj + eq_penalty + ineq_penalty

    # MPPI helper - step function
    def _create_mppi_step(self):
        """Create MPPI step function."""
        lb_np = np.asarray(self.lb)
        ub_np = np.asarray(self.ub)
        ...
        def mppi_step(key, x_mean, cs):
            ...
            samples = np.clip(samples, lb_np, ub_np)
            ...
            exp_weights = np.exp(-(costs - min_cost) / temperature)
            weights = exp_weights / np.sum(exp_weights)
            
            # Weighted mean update
            x_new = np.sum(weights[:, None] * samples, axis=0)
            
            return key, x_new, np.mean(costs)
    ...
    def run_control_space_optimization(self, options: dict | None = None):
        ...
        info['obj_val'] = float(self.compute_objective(np.asarray(x_sol), self.current_state))
    ...
    def run_single_optimization(self, options: dict | None = None, seed: int | None = None):
        ...
        x_mean = np.asarray(x0, dtype=np.float64)
        ...
        info = {
            ...
            'obj_val': float(self.compute_objective(np.asarray(x_sol), self.current_state)),
    ...
    def run_repeated_optimizations(self, warm_start: bool = True, options: dict | None = None):
        ...
        for k in range(n_steps):
            ...
            if warm_start:
                self.current_state.use_virtual_floor = np.array(True)  # if you want virtual-floor warm start
                # Warm-start solve
                t_ws0 = time.perf_counter()
                x_ws, info_ws = self.run_single_optimization(options)
                sv_ws = self._unravel(np.asarray(x_ws))
            ...
            self.current_state.use_virtual_floor = np.array(False)
            ...
            final_obj = self.compute_objective(np.asarray(x_sol), self.current_state)
            print(f"Optimization {k+1}/{n_steps}: objective = {float(final_obj):.6f}")

            sv_sol = self._unravel(np.asarray(x_sol))
...
def update_state_from_solution(problem: TAMOLS, sv_sol):
    ...
    return CurrentState(
        ...
        p_3_meas=p3m,
        p_4_meas=p4m, 
        current_base_pose=np.concatenate([p_end, r_end]),
        current_base_velocity=np.concatenate([v_end, w_end]),
        use_virtual_floor=np.array(False),
    )


def build_initial_x0(gait: Gait, state: CurrentState, terrain: Terrain) -> np.ndarray:
    a_pos = np.zeros((gait.n_phases, 3, gait.spline_order + 1))
    a_rot = np.zeros((gait.n_phases, 3, gait.spline_order + 1))

    # Seed constant terms with current base pose/orientation
    a_pos[:, :, 0] = state.current_base_pose[:3]
    a_rot[:, :, 0] = state.current_base_pose[3:]

    # Phase-0 velocity from current state (to satisfy ICs better), others from desired
    a_pos[0, :, 1] = state.current_base_velocity[:3]
    a_rot[0, :, 1] = state.current_base_velocity[3:]
    if gait.n_phases > 1:
        a_pos[1:, :, 1] = gait.desired_base_velocity
        a_rot[1:, :, 1] = gait.desired_base_angular_velocity
    ...
    def shift_and_project(p):
        ...
        z_h = np.where(np.isnan(z_h), p[2], z_h)
        p_xy[2] = z_h
        return p_xy
    ...
    tpl = {
        ...
        "slack_var": np.full(gait.n_phases, 1e-4),
    }
